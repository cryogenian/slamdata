  ...
  settings ss = case ss of
    Left initialState ->
      HH.slot' cpMongoDB unit \_ ->
        { component: MongoDB.comp requestNewIdTokenBus, initialState }
    Right initialState ->
      HH.slot' cpSQL unit \_ ->
        { component: SQL2.comp requestNewIdTokenBus, initialState: H.parentState initialState }
...
eval requestNewIdTokenBus (Save k) = do
  { parent, name, new } <- H.get
  H.modify (_saving .~ true)
  newName <-
    if new then Api.getNewName requestNewIdTokenBus parent name else pure (pure name)
  case newName of
    Left err → do
