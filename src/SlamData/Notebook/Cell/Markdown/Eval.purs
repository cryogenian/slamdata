...
module SlamData.Notebook.Cell.Markdown.Eval
  ( markdownEval
  , markdownSetup
  ) where
...
import SlamData.Notebook.Cell.Ace.Component as ACE
...
markdownEval
  ∷ CEQ.CellEvalInput
  → String
  → ACE.AceDSL CEQ.CellEvalResult
markdownEval { cellId, notebookPath } str =
  AffF.fromAff do
    result ← Aff.attempt ∘ evalEmbeddedQueries notebookPath cellId $ SDP.parseMd str
    pure
      case result of
        Left err →
...
evalEmbeddedQueries dir cellId =
  ...
  runQuery code =
    case dir of
      ...
      Just dir' → do
        n ← freshInt
        let tempPath = dir' </> Path.file ("tmp" ⊕ CID.cellIdToString cellId ⊕ "-" ⊕ show n)
