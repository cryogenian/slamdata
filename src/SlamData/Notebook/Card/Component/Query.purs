...
_CardEvalQuery ∷ forall a. PrismP (InnerCardQuery a) (CardEvalQuery a)
_CardEvalQuery = _Left

_AnyCardQuery ∷ forall a. PrismP (InnerCardQuery a) (AnyCardQuery a)
...
_AceQuery ∷ forall a. PrismP (AnyCardQuery a) (Ace.QueryP a)
_AceQuery = prism' AceQuery \q → case q of
  AceQuery q' → Just q'
  _ → Nothing

_ExploreQuery ∷ forall a. PrismP (AnyCardQuery a) (Explore.QueryP a)
_ExploreQuery = prism' ExploreQuery \q → case q of
  ExploreQuery q' → Just q'
  _ → Nothing

_MarkdownQuery ∷ forall a. PrismP (AnyCardQuery a) (Markdown.QueryP a)
_MarkdownQuery = prism' MarkdownQuery \q → case q of
  MarkdownQuery q' → Just q'
  _ → Nothing

_SearchQuery ∷ forall a. PrismP (AnyCardQuery a) (Search.Query a)
_SearchQuery = prism' SearchQuery \q → case q of
  SearchQuery q' → Just q'
  _ → Nothing

_JTableQuery ∷ forall a. PrismP (AnyCardQuery a) (JTable.QueryP a)
_JTableQuery = prism' JTableQuery \q → case q of
  JTableQuery q' → Just q'
  _ → Nothing

_VizQuery ∷ forall a. PrismP (AnyCardQuery a) (Viz.QueryP a)
_VizQuery = prism' VizQuery \q → case q of
  VizQuery q' → Just q'
  _ → Nothing

_ChartQuery ∷ forall a. PrismP (AnyCardQuery a) (Chart.QueryP a)
_ChartQuery = prism' ChartQuery \q → case q of
  ChartQuery q' → Just q'
  _ → Nothing

_DownloadQuery ∷ forall a. PrismP (AnyCardQuery a) (Download.QueryP a)
_DownloadQuery = prism' DownloadQuery \q → case q of
  DownloadQuery q' → Just q'
  _ → Nothing

_APIQuery ∷ forall a. PrismP (AnyCardQuery a) (API.QueryP a)
_APIQuery = prism' APIQuery \q → case q of
  APIQuery q' → Just q'
  _ → Nothing

_APIResultsQuery ∷ forall a. PrismP (AnyCardQuery a) (APIResults.QueryP a)
_APIResultsQuery = prism' APIResultsQuery \q → case q of
  APIResultsQuery q' → Just q'
  _ → Nothing

_NextQuery ∷ forall a. PrismP (AnyCardQuery a) (Next.QueryP a)
_NextQuery = prism' NextQuery \q → case q of
  NextQuery q' → Just q'
  _ → Nothing

_SaveQuery ∷ forall a. PrismP (AnyCardQuery a) (Save.QueryP a)
