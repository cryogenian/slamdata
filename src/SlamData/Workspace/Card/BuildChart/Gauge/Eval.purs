...
module SlamData.Workspace.Card.BuildChart.Gauge.Eval
  ( eval
  , module SlamData.Workspace.Card.BuildChart.Gauge.Model
  ) where
...
eval (Just conf) tr@{resource} = do
  ...
  pure $ Port.ChartInstructions tr (buildGauge conf records) Gauge
