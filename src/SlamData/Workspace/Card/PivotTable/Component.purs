...
module SlamData.Workspace.Card.PivotTable.Component where
...
component = CC.makeCardComponent CT.PivotTable $ H.component
...
render st =
  ...
  pageField currentPage customPage totalPages =
    HH.div_
      [ HH.form
          [ HE.onSubmit (HE.input \s → right ∘ UpdatePage s) ]
          [ HH.text "Page"
          , HH.input
              [ HP.type_ HP.InputNumber
              , HP.value (fromMaybe (show (currentPage + 1)) customPage)
              , HE.onValueInput (HE.input \s → right ∘ SetCustomPage s)
              ]
  ...
  pageSizeControls pageSize =
    ...
    in
      HH.div_
        [ HH.select
            [ HE.onValueChange (HE.input \s → right ∘ ChangePageSize s) ]
            options
        ]
...
cardEval = case _ of
  ...
  CC.Save k → do
    { pageSize } ← H.get
    pure $ k $ Card.PivotTable { pageSize }
  CC.Load model next → do
    case model of
      Card.PivotTable m →
        H.modify _{ pageSize = m.pageSize }
      _ → pure unit
    pure next
