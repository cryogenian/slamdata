...
updatePort = case _ of
  P.Blocked →
    H.modify $ _message ?~ "There are no available next actions"
  P.CardError _ →
    H.modify $ _message ?~ "There are no available next actions (parent cards have errors)"
  P.DownloadOptions _ →
    H.modify
      $ (_types .~ [ Ct.Download ])
      ∘ (_message .~ Nothing)
  ...
  P.TaggedResource _ →
    H.modify
      $ (_types .~ [ Ct.DownloadOptions, Ct.Search, Ct.Ace Ct.SQLMode, Ct.Viz, Ct.Save ])
      ∘ (_message .~ Nothing)
