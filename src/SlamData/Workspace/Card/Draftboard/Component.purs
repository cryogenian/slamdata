...
draftboardComponent ∷ RequestIdTokenBus → CardOptions → CC.CardComponent
draftboardComponent requestNewIdTokenBus opts = CC.makeCardComponent
  { cardType: CT.Draftboard
  , component: H.parentComponent
      { render: render opts
      , eval: coproduct evalCard (evalBoard requestNewIdTokenBus opts)
      , peek: Just (peek requestNewIdTokenBus opts)
      }
...
evalBoard ∷ RequestIdTokenBus → CardOptions → Query ~> DraftboardDSL
      ...
      Drag.Move _ d → do
        ...
        for_ st.canvas \el → do
          ...
          H.modify
            ...
            ∘ (_grouping .~ grouping)
      Drag.Done _ → do
        stopDragging requestNewIdTokenBus opts
...
peek ∷ ∀ a. RequestIdTokenBus → CardOptions → H.ChildF DeckId DNQ.QueryP a → DraftboardDSL Unit
...
stopDragging ∷ RequestIdTokenBus → CardOptions → DraftboardDSL Unit
stopDragging requestNewIdTokenBus opts = do
  ...
  for_ st.moving \(deckId × rect) → do
    ...
    case st.grouping of
      Just deckId' →
        groupDecks requestNewIdTokenBus opts deckId deckId'
      Nothing →
...
addDeck ∷ RequestIdTokenBus → CardOptions → DM.Deck → { x ∷ Number, y ∷ Number } → DraftboardDSL Unit
...
addDeckAt ∷ RequestIdTokenBus → CardOptions → DM.Deck → DeckPosition → DraftboardDSL Unit
addDeckAt requestNewIdTokenBus { deck: opts, deckId: parentId, cardId } deck deckPos = do
  ...
  putDeck requestNewIdTokenBus opts.path deckId deck' opts.wiring.decks >>= case _ of
    Left err → do
...
deleteDeck ∷ RequestIdTokenBus → CardOptions → DeckId → DraftboardDSL Unit
...
wrapDeck ∷ RequestIdTokenBus → CardOptions → DeckId → DraftboardDSL Unit
wrapDeck requestNewIdTokenBus { cardId, deckId: parentId, deck } oldId = do
  H.gets (Map.lookup oldId ∘ _.decks) >>= traverse_ \deckPos → do
    ...
    putDeck requestNewIdTokenBus deck.path newId newDeck deck.wiring.decks >>= case _ of
      Left err →
...
unwrapDeck
  ...
  → DraftboardDSL Unit
unwrapDeck requestNewIdTokenBus { deckId, cardId, deck: opts } oldId decks = void $ runMaybeT do
...
mirrorDeck ∷ RequestIdTokenBus → CardOptions → DeckId → DraftboardDSL Unit
  ...
  queryDeck oldId (H.request DCQ.GetModelCards) >>= traverse_ \modelCards → do
    ...
    if Array.null modelCards'.rest
      then insertNewDeck DM.emptyDeck { mirror = DCS.coordModelToCoord <$> modelCards'.init }
        ...
        newId ← H.fromEff freshDeckId
        putDeck requestNewIdTokenBus opts.deck.path newId newDeck opts.deck.wiring.decks >>= case _ of
          Left err →
...
groupDecks ∷ RequestIdTokenBus → CardOptions → DeckId → DeckId → DraftboardDSL Unit
