...
render ∷ ∀ r. RequestIdTokenBus r → DeckOptions → (DeckOptions → DeckComponent) → DCS.State → DeckHTML
...
eval ∷ ∀ r. RequestIdTokenBus r → DeckOptions → Query ~> DeckDSL
eval requestNewIdTokenBus opts@{ wiring } = case _ of
  ...
  Load dir deckId next → do
    loadDeck requestNewIdTokenBus opts dir deckId
    pure next
  ...
  ExploreFile res next → do
    ...
    saveDeck requestNewIdTokenBus opts Nothing
    updateIndicator
    pure next
  Publish next → do
  ...
  GetParent k →
    k <$> H.gets _.parent
  Save coord next →
    saveDeck requestNewIdTokenBus opts coord $> next
  RunPendingCards { source, pendingCard, cards } next → do
    ...
    when (any (DCS.eqCoordModel pendingCoord) st.modelCards) do
      runPendingCards requestNewIdTokenBus opts source pendingCard cards
    pure next
  ...
  GetVarMaps k → do
    ...
    k <$> getVarMaps requestNewIdTokenBus deckPath wiring
  FlipDeck next → do
    updateBackSide opts
    H.modify
      $ DCS._displayMode
...
getVarMaps ∷ ∀ r. RequestIdTokenBus r → DirPath → Wiring → DeckDSL (Map.Map DeckId Port.VarMap)
getVarMaps requestNewIdTokenBus path wiring =
  ...
  goDeck acc deckId = do
    res ← runExceptT do
      deck ← ExceptT $ getDeck requestNewIdTokenBus path deckId wiring.decks
      mirroredCards ← ExceptT $ loadMirroredCards requestNewIdTokenBus wiring path deck.mirror
...
peek ∷ ∀ a r. RequestIdTokenBus r → DeckOptions → H.ChildF ChildSlot ChildQuery a → DeckDSL Unit
...
peekDialog ∷ ∀ a r. RequestIdTokenBus r → DeckOptions → Dialog.Query a → DeckDSL Unit
...
peekDialog _ _ (Dialog.FlipToFront _) =
  H.modify (DCS._displayMode .~ DCS.Normal)
peekDialog requestNewIdTokenBus opts (Dialog.SetDeckName name _) =
  H.modify ((DCS._displayMode .~ DCS.Normal) ∘ (DCS._name .~ name))
    *> saveDeck requestNewIdTokenBus opts Nothing
peekDialog _ _ (Dialog.Confirm d b _) = do
  H.modify (DCS._displayMode .~ DCS.Backside)
  case d of
    Dialog.DeleteDeck | b → raise' $ H.action $ DoAction DeleteDeck
    _ → pure unit

peekBackSide ∷ ∀ a r. RequestIdTokenBus r → DeckOptions → Back.Query a → DeckDSL Unit
peekBackSide requestNewIdTokenBus opts (Back.DoAction action _) =
  case action of
        ...
        case snd trashId of
          ...
          CardId _ → do
            ...
            DBC.childDeckIds (snd <$> fst rem)
              # (H.fromAff :: Slam ~> DeckDSL) ∘ runParallel ∘ traverse_ (parallel ∘ DBC.deleteGraph requestNewIdTokenBus state.path)
            H.set $ snd rem
            triggerSave Nothing
    ...
    Back.Embed → do
      ...
      sharingInput ← getSharingInput
      varMaps ← getVarMaps requestNewIdTokenBus (DCS.deckPath st) opts.wiring
...
runPendingCards
  ...
  → DeckDSL Unit
runPendingCards requestNewIdTokenBus opts source pendingCard pendingCards = do
  st ← H.get
  let
      ...
      step ←
        getCache (DCS.coordModelToCoord c) pendingCards >>= case _ of
          ...
          Nothing → do
            ...
            ev ← evalCard requestNewIdTokenBus opts.wiring.analytics st.path urlVarMaps input c
...
evalCard
  ...
  → DeckId × Card.Model
  → DeckDSL CardEval
evalCard requestNewIdTokenBus bus path urlVarMaps input card = do
  output ← H.fromAff $ Pr.defer do
    ...
    case Card.modelToEval model of
      ...
      Right cmd → do
        out@(Tuple p _) ← flip (Eval.runEvalCard requestNewIdTokenBus) cmd
          { path
          , urlVarMaps
...
saveDeck ∷ ∀ r. RequestIdTokenBus r → DeckOptions → Maybe (DeckId × CardId) → DeckDSL Unit
saveDeck requestNewIdTokenBus { accessType, wiring, cursor } coord = do
  ...
  saveMainDeck st modelCards = do
    ...
    when (isNothing st.parent) do
      ...
      WM.getRoot requestNewIdTokenBus index >>= case _ of
        Left _ → void $ WM.setRoot requestNewIdTokenBus index st.id
        Right _ → pure unit

    putDeck requestNewIdTokenBus st.path st.id model wiring.decks >>= case _ of
      Left err →
  ...
  saveMirroredCard st (deckId × card) =
    getDeck requestNewIdTokenBus st.path deckId wiring.decks >>= case _ of
      Left err →
...
loadDeck ∷ ∀ r. RequestIdTokenBus r → DeckOptions → DirPath → DeckId → DeckDSL Unit
loadDeck requestNewIdTokenBus opts path deckId = do
  ...
  res ← runExceptT do
    deck ← ExceptT $ getDeck requestNewIdTokenBus path deckId opts.wiring.decks
    mirroredCards ← ExceptT $ loadMirroredCards requestNewIdTokenBus opts.wiring path deck.mirror
...
loadMirroredCards requestNewIdTokenBus wiring path coords = (H.fromAff :: Slam ~> DeckDSL) do
  let deckIds = Array.nub (fst <$> coords)
  res ← sequence <$> runParallel (traverse (parallel ∘ flip (getDeck requestNewIdTokenBus path) wiring.decks) deckIds)
