...
comp requestNewIdTokenBus = H.parentComponent { render, eval, peek: Just (peek ∘ H.runChildF) }
  where
  render ∷ State → HTML
  render state =
    HH.div
    ...
    dialog (Embed sharingInput varMaps) =
      HH.slot' cpEmbed unit \_ →
        { component: Export.comp requestNewIdTokenBus
        , initialState:
