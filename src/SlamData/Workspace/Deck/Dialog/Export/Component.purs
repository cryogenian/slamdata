...
eval requestNewIdTokenBus (Revoke next) = do
  mbPermToken ← H.gets _.permToken
  for_ mbPermToken \tok → do
    H.modify _{submitting = true}
    deleteRes ← Q.deleteToken requestNewIdTokenBus tok.id
    H.modify _{submitting = false}
    case deleteRes of
      Left _ → H.modify _{ errored = true }
...
eval requestNewIdTokenBus (ToggleShouldGenerateToken next) = next <$ do
  state ← H.get
    ...
    Nothing →
      unless state.shouldGenerateToken do
        ...
        for_ mbOIDC \oidc → do
          ...
          H.modify _{submitting = true}
          recreatedRes ← Q.createToken requestNewIdTokenBus tokenName actions
          H.modify _{submitting = false}
