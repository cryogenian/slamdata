...
eval requestNewIdTokenBus (Init next) = next <$ do
  ...
  permsRes ← Q.permissionList requestNewIdTokenBus false
  case permsRes of
    Left e → H.modify (_{errored = true})
    Right ps →
      let
...
eval requestNewIdTokenBus (PermissionResumeChanged name string next) = next <$ do
  state ← H.get
  ...
  for_ mbUserPerms \perms → do
    ...
    (succeeded × actionMap) ←
      changePermissionResumeForUser requestNewIdTokenBus name state.sharingInput newResume perms
    H.modify
  ...
  for_ mbGroupPerms \perms → do
    ...
    (succeeded × actionMap)  ←
      changePermissionResumeForGroup requestNewIdTokenBus name state.sharingInput newResume perms
    H.modify
...
eval requestNewIdTokenBus (UnshareToken tokenId next) = next <$ do
  state ← H.get
  let
  ...
  for_ mtix \tix → do
    ...
    Q.deleteToken requestNewIdTokenBus tokenId >>= case _ of
      Left _ →
        H.modify
...
eval requestNewIdTokenBus (Unshare name next) = next <$ do
  state ← H.get
  H.modify (_{errored = false})
  let
    mbUserPerms = SM.lookup name state.userPermissions
  ...
  for_ mbUserPerms \userPerms → do
    ...
    leftPids ← deletePermission requestNewIdTokenBus userPerms.actions
    H.modify
  ...
  for_ mbGroupPerms \groupPerms → do
    ...
    leftPids ← deletePermission requestNewIdTokenBus groupPerms.actions
    H.modify
      if Map.isEmpty leftPids
        then
        (_groupPermissions ∘ ix name ∘ _state .~ Nothing)
...
changePermissionResumeForUser requestNewIdTokenBus name sharingInput res perm = do
  leftPids ← deletePermission requestNewIdTokenBus perm.actions
  if not $ Map.isEmpty leftPids
    ...
    shareRes ← Q.sharePermission requestNewIdTokenBus shareRequest
    case shareRes of
...
changePermissionResumeForGroup
  ...
  → Permission
  → DSL (Boolean × (Map.Map QTA.PermissionId QTA.ActionR))
changePermissionResumeForGroup requestNewIdTokenBus name sharingInput res perm =
