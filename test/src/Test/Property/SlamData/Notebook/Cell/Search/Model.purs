...
module Test.Property.SlamData.Notebook.Cell.Search.Model
  ( ArbModel()
  , runArbModel
  , check
  ) where
...
import Test.Property.SlamData.FileSystem.Resource (runArbResource)
...
instance arbitraryArbModel :: Arbitrary ArbModel where
  arbitrary = do
    file <- map runArbResource <$> arbitrary
    input <- arbitrary
    pure $ ArbModel { file, input }
...
check = quickCheck $ runArbModel >>> \model ->
  case M.decode (M.encode model) of
    ...
    Right model' ->
      mconcat
       [ model.file == model'.file <?> "file mismatch"
